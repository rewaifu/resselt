...
class MoSRArch(Architecture[MoSR]):
    ...
    def load(self, state: Mapping[str, object]) -> WrappedModel:
        ...
        model = MoSR(in_ch=in_ch, out_ch=out_ch, n_block=n_block, upscale=upscale, dim=dim, upsampler=upsampler,
                     expansion_ratio=expansion_ratio, conv_ratio=conv_ratio, kernel_size=kernel_size)

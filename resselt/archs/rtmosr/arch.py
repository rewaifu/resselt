...
class RepConv(nn.Module):
    ...
    def fuse(self):
        ...
        sum_weight = (self.alpha[0] * pad_conv_1x1 + self.alpha[1] * self.conv2.weight + self.conv3.eval_conv.weight * self.alpha[2]).to(device)
        sum_bias = (self.alpha[0] * self.conv1.bias + self.alpha[1] * self.conv2.bias + self.alpha[2] * self.conv3.eval_conv.bias).to(device)
...
class RealTimeMoSR(nn.Module):
    def __init__(self, scale=4, dim=32, ffn_expansion=1.5, n_blocks=2, unshuffle_mod=False):
        self.scale = scale
        super().__init__()
        unshuffle = 0
        ...
        self.to_feat = (
            nn.Conv2d(3, dim, 3, 1, 1) if not unshuffle else nn.Sequential(nn.PixelUnshuffle(unshuffle), nn.Conv2d(3 * unshuffle**2, dim, 3, 1, 1))
        )
        self.body = nn.Sequential(*[GatedCNNBlock(dim, ffn_expansion) for _ in range(n_blocks)])

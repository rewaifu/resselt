...
SampleMods = Literal['conv', 'pixelshuffledirect', 'pixelshuffle', 'nearest+conv', 'dysample', 'pa_up']
...
class DySample(nn.Module):
    ...
    def __init__(self, in_channels: int = 64, out_ch: int = 3, scale: int = 2, groups: int = 4, end_convolution: bool = True, end_kernel=3) -> None:
        super().__init__()
...
class PA(nn.Module):
    def __init__(self, dim, rep) -> None:
        super().__init__()
        self.conv = nn.Sequential(ConvNXC(dim, dim, (1, 1)) if rep else DOConv2d(dim, dim, 1), nn.Sigmoid())
...
class UniUpsampleV4_light(nn.Sequential):
    def __init__(
        ...
        if scale == 1 or upsample == 'conv':
            m.append(ConvNXC(in_dim, out_dim, (3, 3)) if rep else DOConv2d(in_dim, out_dim, 3))
        elif upsample == 'pixelshuffledirect':
            m.extend([ConvNXC(in_dim, out_dim * scale**2, (3, 3)) if rep else DOConv2d(in_dim, out_dim * scale**2, 3), nn.PixelShuffle(scale)])
        elif upsample == 'pixelshuffle':
            m.extend([ConvNXC(in_dim, mid_dim, (3, 3)) if rep else DOConv2d(in_dim, mid_dim, 3), nn.LeakyReLU(inplace=True)])
            if (scale & (scale - 1)) == 0:  # scale = 2^n
                for _ in range(int(math.log2(scale))):
                    m.extend([ConvNXC(mid_dim, mid_dim * 4, (3, 3)) if rep else DOConv2d(mid_dim, 4 * mid_dim, 3), nn.PixelShuffle(2)])
            elif scale == 3:
                m.extend([ConvNXC(mid_dim, mid_dim * 9, (3, 3)) if rep else DOConv2d(mid_dim, 9 * mid_dim, 3), nn.PixelShuffle(3)])
            else:
                raise ValueError(f'scale {scale} is not supported. Supported scales: 2^n and 3.')
            m.append(ConvNXC(mid_dim, out_dim, (3, 3)) if rep else DOConv2d(mid_dim, out_dim, 3))
        elif upsample == 'nearest+conv':
            if (scale & (scale - 1)) == 0:
                for _ in range(int(math.log2(scale))):
                    m.extend((
                        ConvNXC(in_dim, in_dim, (3, 3)) if rep else DOConv2d(in_dim, in_dim, 3),
                        nn.Upsample(scale_factor=2),
                        nn.LeakyReLU(negative_slope=0.2, inplace=True),
                    ))
                m.extend((ConvNXC(in_dim, in_dim, (3, 3)) if rep else DOConv2d(in_dim, in_dim, 3), nn.LeakyReLU(negative_slope=0.2, inplace=True)))
            elif scale == 3:
                m.extend((
                    ConvNXC(in_dim, in_dim, (3, 3)) if rep else DOConv2d(in_dim, in_dim, 3),
            ...
            if mid_dim != in_dim:
                m.extend([ConvNXC(in_dim, out_dim, (3, 3)) if rep else DOConv2d(in_dim, mid_dim, 3), nn.LeakyReLU(inplace=True)])
            ...
            if (scale & (scale - 1)) == 0:
                for _ in range(int(math.log2(scale))):
                    m.extend([
                        nn.Upsample(scale_factor=2),
                        ConvNXC(in_dim, mid_dim, (3, 3)) if rep else DOConv2d(in_dim, mid_dim, 3),
                        PA(mid_dim, rep),
                        nn.LeakyReLU(negative_slope=0.2, inplace=True),
                        ConvNXC(mid_dim, mid_dim, (3, 3)) if rep else DOConv2d(mid_dim, mid_dim, 3),
                        nn.LeakyReLU(negative_slope=0.2, inplace=True),
                    ])
                    in_dim = mid_dim
            elif scale == 3:
...
class DOConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding=None, groups=1, bias=True, dilation=1, mul=1.0):
        ...
        self.padding = get_same_padding(self.kernel_size, self.dilation) if padding is None else _pair(padding)
        self.groups = groups
        ...
        self.eval_conv = nn.Conv2d(
            in_channels, out_channels, self.kernel_size, stride=self.stride, padding=self.padding, dilation=self.dilation, groups=self.groups
        )
    ...
    def update_eval(self):
        self.eval_conv.weight.data = (self.dow_func() * self.mul).clone().detach().contiguous()
    ...
    def train_forward(self, x):
        return F.conv2d(x, self.dow_func() * self.mul, self.bias * self.mul, self.stride, self.padding, self.dilation, self.groups)
...
class ConvNXC(nn.Module):
    def __init__(self, c_in: int, c_out: int, kernel_size: tuple[int, int] = (3, 3), gain1: int = 2, mul: int = 1) -> None:
        super().__init__()
        ...
        self.sk = DOConv2d(in_channels=c_in, out_channels=c_out, kernel_size=1, padding=0, mul=0.02)
        ...
        self.conv = nn.Sequential(
            DOConv2d(in_channels=c_in, out_channels=c_in * gain, kernel_size=1, padding=0),
            DOConv2d(in_channels=c_in * gain, out_channels=c_out * gain, kernel_size=kernel_size, padding=0),
            DOConv2d(in_channels=c_out * gain, out_channels=c_out, kernel_size=1, padding=0, mul=mul),
        )
    ...
    def update_params(self) -> None:
        ...
        w = F.conv2d(w1.flip(2, 3).permute(1, 0, 2, 3), w2, padding=(pad_h, pad_w), stride=1).flip(2, 3).permute(1, 0, 2, 3)
        ...
        sk_w = F.pad(sk_w, [w_pixels_to_pad, w_pixels_to_pad, h_pixels_to_pad, h_pixels_to_pad])
...
class SMB(nn.Module):
    def __init__(self, in_channels, out_channels=None, rep=False):
        ...
        self.body = nn.Sequential(
            ConvNXC(in_channels, out_channels, (3, 3), mul=3) if rep else DOConv2d(in_channels, out_channels, 3, mul=3),
            nn.SiLU(True),
            ConvNXC(out_channels, out_channels, (3, 3), mul=3) if rep else DOConv2d(out_channels, out_channels, 3, mul=3),
            nn.SiLU(True),
            ConvNXC(out_channels, out_channels * 2, (3, 3), mul=3) if rep else DOConv2d(out_channels, out_channels * 2, 3, mul=3),
        )
...
class SMoSR(nn.Module):
    def __init__(
        ...
        self.end_block = nn.Sequential(SMB(dim, rep=rep), ConvNXC(dim, dim, (3, 3), mul=1) if rep else DOConv2d(dim, dim, 3))
        self.upsampler = UniUpsampleV4_light(upsampler, scale, dim + in_ch * scale * scale, out_ch, upsampler_mid_dim, 4, d_kernel, rep)
        self.scale = scale * 2

...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        real = torch.unsqueeze(torch.real(ffted), -1)
        imag = torch.unsqueeze(torch.imag(ffted), -1)
        ffted = torch.cat((real, imag), dim=1)

        ffted = ffted.contiguous().view(b, c * 2, h, -1).to(orig_dtype)
        ...
        ffted = F.gelu(ffted)
        ffted = ffted.view(b, c, 2, h, -1)
        ffted = ffted.permute(0, 1, 3, 4, 2).float()
        ffted = torch.view_as_complex(ffted.contiguous())

        output = torch.fft.irfft2(ffted, s=(h, w), norm="ortho")
        output = self.post_norm(output.to(orig_dtype))
        return output
...
class InceptionDWConv2d(nn.Module):
    ...
    def __init__(
        ...
        # для быстрого доступа при инференсе
        self.slices = []
        s = 0
    ...
    def forward(self, x):
        if self.training:
            # режим тренировки — обычный split + cat
            ...
            # режим вывода — обновляем по каналам inplace
            (s0, e0), (s1, e1), (s2, e2), (s3, e3) = self.slices

            x[:, s0:e0] = self.pconv(x[:, s0:e0])
            x[:, s1:e1] = self.dwconv_hw(x[:, s1:e1])
            x[:, s2:e2] = self.dwconv_w(x[:, s2:e2])
            x[:, s3:e3] = self.dwconv_h(x[:, s3:e3])
...
class PadPixelUnshuffle(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.pixel_unshuffle(
            F.pad(x, (0, mod_pad_w, 0, mod_pad_h), "reflect"), self.ps
        )
        ...
        **kwargs,
        ...
        if pixel_unshuffle and scale in [1, 2]:
            ...
            self.in_to_dim = nn.Sequential(
                PadPixelUnshuffle(down), nn.Conv2d(in_nc * down * down, dim, 3, 1, 1)
            )
            scale = 4
        else:

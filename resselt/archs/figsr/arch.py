...
class RMSNorm(nn.Module):
    def __init__(self, dim: int, eps: float = 1e-6) -> None:
        ...
        self.rms = nn.Parameter(torch.Tensor(torch.ones(1) * (dim**-0.5)), requires_grad=False)

    def forward(self, x: Tensor) -> Tensor:
        norm_x = torch.addcmul(self.eps, x.norm(2, dim=1, keepdim=True), self.rms)
        return torch.addcmul(self.offset[:, None, None], x.div(norm_x), self.scale[:, None, None])
...
class InceptionConv2d(nn.Module):
    ...
    def forward(self, x: Tensor, x_hw: Tensor, x_w: Tensor, xh: Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor]:
...
class GatedCNNBlock(nn.Module):
    def __init__(
        ...
        self.conv = InceptionConv2d(dim - gc * 3, gc, square_kernel_size, band_kernel_size)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.gfisr_body_half = nn.Sequential(
            *[GatedCNNBlock(dim, expansion_ratio, gc, square_kernel_size, band_kernel_size) for _ in range(n_blocks // 2)]
        )
        self.gfisr_body_half_2 = nn.Sequential(
            *[GatedCNNBlock(dim, expansion_ratio, gc, square_kernel_size, band_kernel_size) for _ in range(n_blocks - n_blocks // 2)]
            + [nn.Conv2d(dim, dim, 3, 1, 1)]
        )
        self.cat_to_dim = nn.Conv2d(dim * 3, dim, 1)
        self.upscale = UniUpsampleV3(upsampler, scale, dim, out_nc, mid_dim, dysample_end_kernel=3)
